...
def upgrade() -> None:
    ...
    existing_types = conn.execute(sa.text(
        "SELECT typname FROM pg_type WHERE typname = 'jobstatus'"
    )).fetchall()
    
    if not existing_types:
        # Create the enum type only if it doesn't exist
        jobstatus_enum = sa.Enum('pending', 'running', 'completed', 'failed', 'cancelled', 'retrying', name='jobstatus')
        jobstatus_enum.create(conn)
...
def downgrade() -> None:
    ...
    if existing_types:
        sa.Enum(name='jobstatus').drop(conn)
